    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if(this.preguntas.count() > 0) {
            ...
            alert("La encuesta no cuenta con preguntas, revisa la configuración en el portal") {
                ...
                getButton(android.support.v7.app.AlertDialog.BUTTON_POSITIVE)?.let { it.setBackgroundColor(Color.WHITE) }
                getButton(android.support.v7.app.AlertDialog.BUTTON_POSITIVE)?.let { it.setTextColor(Color.parseColor("#ff33b5e5")) }
            }
        }
